...
export function createHeaderFrame(
  ...
  const frameConfig = protocolVersion === 'v3' ? FRAME_V3 : FRAME;
  const sid = sessionId ?? (generateSessionId() & 0xFFFF); // 16-bit session ID
  const frame = new Uint8Array(frameConfig.HEADER_SIZE);

  // Magic ("N1" for v2, "N3" for v3)
  frame.set(stringToBytes(frameConfig.HEADER_MAGIC), 0);

  // Version (high 4 bits) + Flags (low 4 bits)
  let flags = 0;
  if (compressed) flags |= FLAG_COMPRESSED;
  if (encrypted) flags |= FLAG_ENCRYPTED;
  const versionFlags = ((frameConfig.CURRENT_VERSION & 0x0F) << 4) | (flags & 0x0F);
...
export function packetize(
  ...
  // Create header frame with appropriate protocol version
