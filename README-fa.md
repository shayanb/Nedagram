...
**انتقال متن از طریق صدا** - یک برنامه وب پیش‌رونده (PWA) که متن را به سیگنال‌های صوتی (ندا) تبدیل می‌کند که می‌توان بین دستگاه‌ها با استفاده از بلندگو و میکروفون (مثلا تماس تلفنی) منتقل کرد. این برنامه پس از اولین بازدید، بدون اینترنت کار می‌کند.

## چرا نداگرام؟

> «ندا» (Neda) در زبان فارسی به معنای صدای بلند، آواز، بانگ، فریاد و پیام غیبی است.
...
- **کاملاً آفلاین** پس از بارگذاری اولیه - بدون سرور، بدون ردیابی، داده‌ای از دستگاه شما خارج نمی‌شود
...
- **تصحیح خطا** - FEC ترکیبی (Reed-Solomon + Convolutional با رمزگشایی Viterbi)
...
### تصحیح خطا (FEC ترکیبی)

*همان روش استفاده شده توسط فضاپیمای Voyager ناسا برای ارتباطات فضای عمیق.*

| پارامتر | مقدار |
|---------|-------|
| کد بیرونی | Reed-Solomon با 16 بایت پریتی |
| کد درونی | Convolutional (k=7، نرخ 2/3 سوراخ‌شده) |
| رمزگشایی Viterbi | تصمیم نرم، 35 سمبل بازگشت |
| درهم‌ریزی | LFSR (x^15 + x^14 + 1) |
| تصحیح خطا | ~8 بایت + خطاهای بیتی اضافی در هر فریم |
...
۲. یک سرور وب لوکال راه‌اندازی کنید:
   ```bash
...
با هدف کمک به مردم برای برقراری ارتباط آزاد در شرایطی که روش‌های معمول کار نمی‌کنند ساخته شده است.

---

**نداگرام** - وقتی اینترنت کار نمی‌کند، صدا راهی پیدا می‌کند. :telephone_receiver:
